...
//Ricerca per genere
SELECT libri.id AS lib_id,ISBN,titolo, anno_pubblicazione, quantita, collane.nome AS collana,
   casa_editrice.nome AS casa_editrice, CONCAT(autori.cognome, ' ', autori.nome) AS autore, stanze.nome AS stanza, espositori.nome AS espositore
FROM libri
  JOIN collane ON libri.id_collana=collane.id
  JOIN casa_editrice ON libri.id_editore=casa_editrice.id
  JOIN libri_autori ON libri.id=libri_autori.id_libro
...
SELECT libri.titolo AS titolo, CONCAT(utenti.cognome, ' ', utenti.nome) AS utente, CONCAT(personale.cognome, ' ', personale.nome) AS personale, data_inizio, data_fine, restituito
FROM prestiti
	JOIN libri ON prestiti.id_libro=libri.id
    JOIN utenti ON prestiti.id_utente=utenti.id
...
SELECT libri.id AS id, ISBN, libri.titolo AS titolo, CONCAT(autori.cognome, ' ', autori.nome) AS autore,collane.nome AS collana, casa_editrice.nome AS casa_editrice,generi.nome AS genere, quantita AS quantita, stanze.nome AS stanza, espositori.nome AS espositore
FROM libri
  JOIN collane ON libri.id_collana=collane.id
  JOIN casa_editrice ON libri.id_editore=casa_editrice.id
  JOIN libri_autori ON libri.id=libri_autori.id_libro
